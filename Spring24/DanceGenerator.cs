...
namespace CustomNPCFestivalAdditions.Spring24
{
    internal class DanceGenerator
    {
        ...
        public static string getMutualDispositionLoveInterest(NPC target)
        {
            try
            {
        ...
        public static void createMutualDispositionLoveInterestPair(List<string> poolUpper, string upperDancer, List<string> poolLower, List<NetDancePartner> upperline, List<NetDancePartner> lowerline)
        {
            if (getMutualDispositionLoveInterest(Game1.getCharacterFromName(upperDancer)) != null)
            {
                string lowerDancer = getMutualDispositionLoveInterest(Game1.getCharacterFromName(upperDancer));

                if (poolLower.Contains(lowerDancer))
                {
                    ...
                    Monitor.Log($"Made a mutual NPC Disposition \"Love Interest\" pair with {upperDancer} and {lowerDancer}, and successfully entered pair into NetDancePartner.", LogLevel.Trace);
                }
                else
                {
                    Monitor.Log($"Could not make a mutual NPC Disposition \"Love Interest\" pair with {upperDancer} and {lowerDancer} as {lowerDancer} is not a valid member of the lower dancer pool. Will attempt to pair {upperDancer} with a random lower dancer that lacks a mutual NPC Disposition \"love interest\".", LogLevel.Trace);

                    int reshuffle = 6;
                    while (reshuffle < 1)
                    {
                        Random rnd2 = new Random();
                        int rLower = rnd2.Next(poolLower.Count);
                        string randomLowerDancer = poolLower[rLower];
                        if (getMutualDispositionLoveInterest(Game1.getCharacterFromName(randomLowerDancer)) == null)
                        {
                            upperline.Add(new NetDancePartner(upperDancer));
                            lowerline.Add(new NetDancePartner(randomLowerDancer));

                            poolUpper.Remove(upperDancer);
                            poolLower.Remove(randomLowerDancer);

                            Monitor.Log($"Randomly made a pair with {upperDancer} and {randomLowerDancer} and successfully entered pair into NetDancePartner.", LogLevel.Trace);
                            break;
                        }
                        else
                        {
            ...
            {
                Monitor.Log($"Could not make a mutual NPC Disposition \"Love Interest\" pair with {upperDancer} as {upperDancer} does not have a listed NPC Disposition \"Love Interest\". Will attempt to pair {upperDancer} with a random lower dancer that lacks a mutual NPC Disposition \"love interest\".", LogLevel.Trace);
        ...
        public static void createRandomNPCPair (List<string> poolUpper, string upperDancer, List<string> poolLower, List<NetDancePartner> upperline, List<NetDancePartner> lowerline)
        {
            Random rnd2 = new Random();
            int rLower = rnd2.Next(poolLower.Count);
            string lowerDancer = poolLower[rLower];
            ...
            Monitor.Log($"Randomly made a pair with {upperDancer} and {lowerDancer} and successfully entered pair into NetDancePartner.", LogLevel.Trace);
        }
